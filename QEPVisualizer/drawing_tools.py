...
def clicked(event,canvas):
    ...
    if node in instance_dict.keys():
        ...
        instance=canvas.create_text((node.center[0] + 130,node.center[1]-(NODE_HEIGHT/2)+50),text=node.text,fill="blue")
...
def draw_query_plan(data):
    ...
    node_list.append(Node(0, CANVAS_WIDTH,150, NODE_HEIGHT))
...
def getInfo(plan):
    ...
    if (plan["Node Type"] == "Aggregate"):
        ...
        parsed_plan += "Duration taken: " + plan["Total Cost"] + "."
        return parsed_plan
